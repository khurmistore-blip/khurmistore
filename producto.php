...
$rows = $id ? sb_get($cfg, "products?id=eq.$id&is_active=is.true&approval_status=eq.approved&limit=1") : [];
$p = $rows[0] ?? null;

// Belt-and-suspenders: even though the query above already filters on
// is_active=is.true&approval_status=eq.approved (so an out-of-stock or
// non-approved product simply won't be fetched), explicitly re-check the
// fetched row's own fields too, so a direct link can never render one.
if ($p && (($p['approval_status'] ?? '') !== 'approved' || ($p['is_active'] ?? false) !== true)) {
    $p = null;
}
